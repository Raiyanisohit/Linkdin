...
async function getallproject() {
  ...
  Alluserproject.value = userproject.Message.reverse();
}
getallproject();
...
watchEffect(() => {
  if (projectEndmonth.value == "") {
    ...
    error.endmonth = "End date can’t be in the future";
  } else {
    error.endmonth = "";
  }
});
...
//End_month && End_Year
  ...
  if ( 
    ...
    projectStartmonth.value != "" &&
    projectStartyear.value != "" &&
    (projectEndyear.value > projectStartyear.value  && (currentYear  >= projectEndyear.value ) ||
      (projectStartyear.value === projectEndyear.value &&
       ( projectStartmonth.value <= projectEndmonth.value && currentMonth <= projectEndmonth.value)) )
  ) {
    const data = JSON.stringify({
      id: deletid.value,
      projectName: projecttitle.value,
    ...
    >
      ...
      >
        <ul class="justify-between">
         ...
         <li v-if="!m.projectEndMonth" class="text-gray-500 text-[14px] pt-1">
          {{ m.projectStartMonth }}  {{ m.projectStartYear }} - Continue
        </li>
        <li v-else class="text-gray-500 text-[14px] pt-1">
          {{ m.projectStartMonth }}  {{ m.projectStartYear }} - {{ m.projectEndMonth }}  {{ m.projectEndYear }}
        </li>
        </ul>
        <ul>
