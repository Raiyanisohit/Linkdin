...
function handleLogin() {
  ...
  if (input.email.trim() != "" && input.password.trim() != "") {
    if (emailpatten.value.test(input.email) != true) {
      ...
      required.password = "Password must contain at least one lowercase letter,one uppercase letter,one digit,one special character and be at least 6 characers long...";
    } else {
      axios
      ...
      <form action="POST" @submit.prevent="handleLogin">
        ...
        <div class="pt-[35px] relative">
          ...
          <p
            ...
            class="text-red-600 font-semibold  font-serif"
          >
            {{ required.password }}
          </p>
        </div>
