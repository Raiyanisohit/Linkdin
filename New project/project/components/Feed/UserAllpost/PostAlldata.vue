...
async function showcomment(id) {
  ...
  console.log(Allshowcomment.Data)
...
async function usercommentt(id) {
  ...
  inputcomment.value = "";
  showallcomment.value.push(comment)
  await getcommentno()
    }
}
    ...
    >
      <div class="flex justify-between px-7 px-2">
        ...
        <div v-if="delet" class="dropdown dropstart relative">
          ...
          <ul class="dropdown-menu absolute mt-3 left-5 ">
            <button data-bs-toggle="modal" data-bs-target="#deletepost" class="dropdown-item hover:text-white hover:text-bold flex hover:bg-red-400">
              <img
                class="w-[25px] mr-2"
      ...
      <div class="flex mt-4 mb-1 justify-between">
        <div class="ps-8 " v-if="postId">
          <p v-if="TotalLikeno > 0">{{ TotalLikeno }} Like</p>
        </div>
        <div  class="pr-3" >
          <button 
          ...
          <div class="flex">
            ...
            <div class=" ps-1 mt-2  mb-3 relative   text-gray-600">
              <input
                ...
                class="bg-white w-[180px] md:w-[400px]  border-secondary border h-10 px-7 pr-10 rounded-full text-sm focus:outline-none"
              />
              <button v-if="inputcomment.trim()" class="rounded-e-full outline-dotted outline-1  border-s-2    absolute right-0 font-bold border-dark  px-3 sm:px-2 p-2  ">POST</button>
            </div>
            <div>
            </div>
          </div>
...
<deletmodal id="deletepost" title="Delete Post?">
 ...
 <button  data-bs-dismiss="modal"  @click="deletepost(postId)"  class="me-1 float-right text-white rounded border-2 bg-red-600 hover:bg-red-700   px-3 py-1 border-danger font-bold">DELETE</button>
